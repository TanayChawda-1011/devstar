<script lang="ts">
  ...
  function reverseIntegerDigits(input) {
    return input
      .split(/\s+/)
      .map((str) => {
        let num = parseInt(str);
        return isNaN(num) ? str : str.split("").reverse().join("");
      })
      .join(" ");
  }
  ...
  $: if (reverseDigits !== undefined) generatePalindrome();
</script>
